...
fc_filtered |>
  autoplot() +
  geom_line(data = data_2017,
  aes(x = Datetime, y = AEP_MW, color ="Testdatensatz")) +
  scale_color_manual(
      values = c(
        "ARIMA" = "red",
        "DRIFT" = "black",
...
ggplot()+
  geom_point(data = result_3, aes(x=RealPowerGeneration, y=.mean, color=WorkDay)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", lwd=1)


ggplot() +
  geom_line(data=data_2017, aes(x=Datetime, y=AEP_MW))
...
fit |> select(ARIMA_A1) |> gg_tsresiduals()
