...
Aktivierungsfunktionen haben einen großen Einfluss auf die Modellierung im Bereich der Neuronalen Netze. Dabei reagieren Neuronen auf verschiede Aktivierungsfunktionen unterschiedlich und beschreiben damit, ob ein Neuron gerade den Status "aktiv" oder "nicht aktiv" hat. Ist ein bestimmter Schwellwert in einer dieser Funktionen überschritten, so ändert das Neuron seinen Status. Die wohl bekanntesten Aktivierungsfunktionen sind sigmoid, relu, tanh und eine linearen Aktivierung. In der Arbeit von \cite{Ramachandran2017} wurde die swish-Funktion aufgedeckt:

\[
f(x) = x * sigmoid(\beta * x)
\]

In der Arbeit von \cite{Ramachandran2017} wurde die swish-Funktion als die beste Aktivierungsfunktion alternativ zu der ReLU Funktion empirisch bestimmt. Die swish-Funktion soll dabei glättere Verläufe ermöglichen, was zu einer stabileren und effizienteren Modelloptmierung führen kann.

## Auto Regressive-Moving Average (ARIMA)

Bei dem (S)ARIMA Modell handelt es sich um ein additives statistisches Modell, welches sich an komplexe Zeitreihe anpassen 
kann. Dabei beschreiben die Therme PDQ (saisonall) und pdq (nicht saisonall) das Modell. Folgende Backshift Notation beschreibt
das Modell:
...
Dabei wird die Zeitreihe d-mal und D-mal Differenziert. Die Therme p,d,P,Q werden mittels des geringsten AICc ermittelt.

## PROPHET: Forecasting at a scale

Das Prophet Model wurde von Facebook entwickelt und findet Anwendung bei Vorhersagen von Zeitreihen mit komplexer Saisonalität.
Speziell designt, um Feiertagmuster zu erkennen. Dabei handelt es sich wie bei dem ARIMA Modell um ein additives Modell. Im Vergleich zum ARIMA Modell, ist das Prophet Model, vor allem im Training deutlich schneller. Hinzu kommt, dass dies Modell im gegensatz zum ARIMA Modell den Effekt der Feiertage deutlich besser erfassen kann. Folgende Formel beschreibt das Modell:
...
s(t) : Die saisonallen Muster

h(t) : Feitagmustererfassung
...
Neben der initialen Variablen, die aus der Datenanalyse erzeugt wurden, hinzufügen wir zwei weitere Spalten, nämlich die wöchentliche und jährliche Werten Verzögerung der abhängigen Variable. Für Time-Series Daten ist eine normale Praktik, die wir hier auch verfolgen. Somit ergeben sich 17, die einen möglichen Einfluss auf das Modell haben können.

Wegen einer großen Dimensionalität des Datensatzes, möchten wir mithilfe der PCA-Analyse zeigen, wie viele Variablen man benötigt um die Varianz der Daten zu erklären. Unter beifügen wir eine Tabelle, wie viele Komponente man braucht um die Varianz des Datensatzes zu erklären:

\begin{table}[h!]
\centering
\begin{tabular}{|c|l|c|}
\hline
\textbf{Principal Component} & \textbf{Merkmal}                        & \textbf{Explained Variance (cumulative)} \\ \hline
...
Die Komponente sind in der Reihenfolge nach deren Wichtigkeit (Feature Importance) platziert. Jedoch man sieht, dass die numerischen Variablen höher platziert sind als die boolische Merkmale, weil diese stärkere Korrelation mit abhängiger Variable aufweisen.
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{plots/best_model_10_epochs_training_no_slaf.png}
\caption{LNN (best): Vorhersagen vs. abhängige Variable}
\label{fig:best_model_10_epochs_training_no_slaf}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{plots/best_model_10_epochs_training_with_slaf.png}
\caption{LNN-SLAF (best): Vorhersagen vs. abhängige Variable}
\label{fig:best_model_10_epochs_training_with_slaf}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{plots/best_model_10_epochs_training_no_slaf_2nd_place.png}
\caption{LNN (2nd best): Vorhersagen vs. abhängige Variable}
\label{fig:best_model_10_epochs_training_no_slaf_2nd_place}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{plots/training_loss_over_epochs.png}
\caption{LNN: Trainingsverluste}
\label{fig:training_loss_over_epochs}
\end{figure}

Die Ergebnisse von den drei besten Modellen werden in dem nächsten Kapitel näher diskutiert.
...
\begin{table}[h!]
    \centering
    \caption{Modellvergleich für eine Vorhersage über das ganze Jahr 2017}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Modell} & \textbf{MAPE} & \textbf{MAE} & \textbf{RMSE} \\
        \midrule
        LNN & 6.82\% & X & X \\
        PROPHET & 7.54\% & 1078 & 1356 \\
        ARIMA & 7.68\% & 1118 &  1421\\
        LNN + SLAF & X\% & X & X \\
        MEAN & 13.72\% & 1888 & 2322 \\
        \bottomrule
    \end{tabular}
\end{table}
