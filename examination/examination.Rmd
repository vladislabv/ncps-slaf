...
Um diese Netze zu trainieren, verwenden die Autoren eine Technik namens Backpropagation through Time (BPTT), beschrieben von \cite{pontryagin2018mathematical}, in Kombination mit einem benutzerdefinierten Solver, um die Gleichungen des Netzes effizient zu lösen. Dieser Solver stellt ein Gleichgewicht zwischen Stabilität und Geschwindigkeit her. Die Aktualisierungsregel lautet wie folgt:

\[
x(t + \Delta t) = x(t) + \Delta t \frac{f(x(t), I(t), t, \theta)A}{1 + \Delta t \left(\frac{1}{\tau} + f(x(t), I(t), t, \theta)\right)}
\]

Die LTCN-Architektur ist wie folgt aufgebaut: jedes Neuron besitzt eine zeitabhängige Aktivierung, die durch die vorher genannte Differentialgleichung definiert wird. Durch die kontinuierlich Anpassung der Zeitkonstanten können LTCNs schnell auf jegliche Veränderungen in den Daten reagieren. Darüber hinaus, können sind LTCNs im Kontext von komplexen, zeitlich veränderlichen Mustern anzuwenden.

In praktischen Tests übertrafen LTCs traditionelle Modelle wie Long Short-Time Memory Cells (LSTMs) und neuronale Ordinary Differential Equation (ODEs). Die Autoren behaupten eine Ergebnissverbesserung beim Benchmark, welcher auf 7 sehr bekannten öffentlichen Datesets basiert. Wie aber andere neuronale Netze haben sie jedoch aufgrund des Problems des verschwindenden Gradienten (Vanishing gradient problem) Schwierigkeiten mit langfristigen Abhängigkeiten. Außerdem benötigen sie im Vergleich zu einfacheren Modellen mehr Rechenressourcen, um eine Vorhersage zu machen. Neber der Neuheit des Modells könnte es erklären, dass praktischen Anwendungen es derzeit noch kaum zu finden sind.

Dennoch macht es LTCs zu einer spannenden Entwicklung in der Welt der rekurrenten neuronalen Netze, auch wenn es noch einige Herausforderungen zu bewältigen gibt.
...
Git-Repo: https://github.com/vladislabv/ncps-slaf.
